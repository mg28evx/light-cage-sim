...
\src{\fn{sample\_henyey\_greenstein}, \code{simulation\_engine.py}, líneas 54--75;
aplicación y atenuación por $\omega$: líneas 617--624.}
...
\nota{La relación clásica de Poole--Atkins $Z_\text{SD}\approx 1{,}7/K_d$~\cite{poole}
se reserva para la \emph{ingesta} de datos satelitales (conversión inversa
$K_d\leftarrow Z_\text{SD}$ en \code{optical\_lookup.py}), no como salida del
simulador: aplicarla sólo al modo $K_d$ —como ocurría antes— rompía la coherencia con
el modo $c$, haciendo que una misma agua arrojara dos profundidades distintas. La
salida unifica ahora todo en la fórmula acoplada de Preisendorfer.}
...
El simulador implementa ambos modelos como opción seleccionable
(\code{secchi\_model} $\in\{$\code{preisendorfer}, \code{lee2015}$\}$). El $K_d(\lambda)$
...
cuando sólo se conoce $c$), y se toma su mínimo como $K_d^{tr}$. Para el modo
\code{bio}, $r_w$ se estima desde las IOP en la ventana transparente con la relación
de Gordon $R(0^-)\approx f\,b_b/(a+b_b)$, $f\approx0{,}33$, usando una
retrodispersión $b_b = B(g)\,b$ con la fracción analítica de la propia fase de
Henyey--Greenstein:
...
\fn{hg\_backscatter\_fraction}, \fn{subsurface\_reflectance} en
\code{simulation\_engine.py} (sección ``Profundidad de disco de Secchi
...
\begin{itemize}[itemsep=3pt]
  ...
  \item \textbf{Una fase de dispersión.} La dispersión volumétrica usa una única
  $g$ (Henyey--Greenstein); no se emplean fases de doble término ni dependencia
  espectral de $g$ salvo la implícita en $\omega(\lambda)$ del modo \code{bio}.
  \item \textbf{Cierre IOP$\to$AOP aproximado.} Las relaciones
  \eqref{eq:kdfromiop}--\eqref{eq:cfromkd} son parametrizaciones de cierre con
  $g\approx0{,}85$ y $\bar\mu_d\approx0{,}85$ fijos.
...
$b_b$ & Coeficiente de retrodispersión, $b_b=B(g)\,b$ & \si{\per\meter} \\
